...
# than legitimate ATS tailoring. Maintainer policy (2026-07): writing to the job
# description IS the product's job, and the default "full" strategy deliberately
# claims JD skills, appends JD-relevant bullets, and quantifies — so the monitor
# must not score that as a regression. This knob ONLY softens the judge's (LLM,
# qualitative) truthfulness lens — the hard structural guards in
# flow.score_tailoring (no_fabricated_employers, personal_info_unchanged) stay
# strict and are NOT affected. Employers, titles, dates, degrees, and
# certifications stay inviolate regardless. Dial this down to tighten the lens.
JD_KEYWORD_TOLERANCE = 0.60
...
_RUBRIC = (  # diverges from tests/evals/test_tailoring_eval.py by design — adds the JD tolerance
    ...
    "Incorporating job-description keywords and skills, adding JD-relevant bullets to "
    "real roles, and quantifying achievements are EXPECTED, legitimate tailoring (ATS "
    "optimization), not fabrication: do NOT lower the score "
    ...
    "titles, dates, degrees, and certifications remain unchanged. DO still penalize "
    "invented employers, fabricated titles, dates, degrees or certifications, and a "
    "wholesale change of profession the master never supported (e.g. a backend engineer "
    "rewritten as a career frontend dev). "
