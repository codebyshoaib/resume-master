...
**Improve prompt selection:** `IMPROVE_RESUME_PROMPTS` = `{nudge, keywords, full}`; `IMPROVE_PROMPT_OPTIONS` is the UI list; `DEFAULT_IMPROVE_PROMPT_ID = "full"`. The active id comes from `config.json` `default_prompt_id` (validated against option ids) or the request's `prompt_id`. `CRITICAL_TRUTHFULNESS_RULES[id]` is injected into each improve prompt via `{critical_truthfulness_rules}`. **`full` is deliberately JD-first** (`AGGRESSIVE_TAILORING_RULES`): it rewrites most bullets and the summary into the JD's language, claims the JD's skills, appends new JD-relevant bullets on real roles, states concrete figures, and matches the JD's seniority/scope language — each surfaced as a suggested addition in the preview→confirm gate. **The only floor is `HARD_FLOOR_RULES`** — employers, job titles, employment/education dates, degrees, certifications, licenses — i.e. exactly the facts a background check resolves; every strategy carries it, and `tests/unit/test_prompt_guardrails.py` fails if it goes missing. `nudge`/`keywords` keep the old evidence ceiling (`CONSERVATIVE_TAILORING_RULES_TEMPLATE`) because a user picks them deliberately for a light touch. Frontend UI default fallback also flips to `full` (`tailor/page.tsx`, `settings/page.tsx`).
