...
**Improve prompt selection:** `IMPROVE_RESUME_PROMPTS` = `{nudge, keywords, full}`; `IMPROVE_PROMPT_OPTIONS` is the UI list; `DEFAULT_IMPROVE_PROMPT_ID = "full"`. The active id comes from `config.json` `default_prompt_id` (validated against option ids) or the request's `prompt_id`. `CRITICAL_TRUTHFULNESS_RULES[id]` is injected into each improve prompt via `{critical_truthfulness_rules}`. **`full` is deliberately aggressive:** it rewrites most bullets into the JD's language AND gap-fills — appends new JD-relevant bullets on real roles and adds JD skills (accepted by `verify_skill_target_plan`, kept through the refiner via `allowed_new_skills`), each surfaced as a suggested addition in the preview→confirm gate. The hard fabrication guards it still enforces: no invented numeric metrics, employers, job titles, or dates. `nudge`/`keywords` remain conservative (no new bullets). Frontend UI default fallback also flips to `full` (`tailor/page.tsx`, `settings/page.tsx`).
